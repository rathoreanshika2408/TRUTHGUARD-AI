      ...
      <div class="tg-body">
        <div class="tg-score-row">
          ...
          <div class="tg-bar-wrap">
            <div class="tg-bar" style="width:${risk.score}%"></div>
          </div>
          <span class="tg-score-num">${risk.score}%</span>
        </div>
        ${risk.triggers.length ? `
        <div class="tg-triggers">
          ${risk.triggers.map(t => `<span class="tg-tag">${t}</span>`).join('')}
        </div>` : ''}
        <div class="tg-actions">
          <span class="tg-tip">Verify before sharing</span>
          <a href="http://127.0.0.1:5500/app.html" target="_blank" class="tg-btn">Verify now →</a>        </div>
      </div>
    </div>`;

  warning.querySelector('.tg-close').addEventListener('click', () => {
    warning.remove();
  });
...
function scanMessages() {
  const selectors = [
    ...
    '.focusable-list-item',
    '._21Ahp',
    '.copyable-area [tabindex="-1"]'
  ];

  selectors.forEach(selector => {
    document.querySelectorAll(selector).forEach(msg => {
      const msgId = msg.textContent.slice(0, 50).trim();
...
function startObserver() {
  const target = document.querySelector(
    '#main, [data-testid="conversation-panel-wrapper"], body'
  );
  if (target) {
